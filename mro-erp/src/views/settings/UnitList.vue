...
function onDrop(event: DragEvent) {
  ...
  const dragIndex = units.value.findIndex(u => u.id === draggedId.value)
  ...
  // Reorder locally (instant visual update)
  const item = units.value.splice(dragIndex, 1)[0]
  units.value.splice(dropIndex, 0, item)
  ...
  try {
    await Promise.all(
      units.value.map((unit, index) =>
        unitsApi.update(unit.id, { sort_order: index })
      )
    )
  } catch (e) {
