    ...
    <div class="max-w-4xl space-y-6">
      ...
      <div v-if="pendingOrders.length > 0">
        <div class="flex items-center gap-2 mb-3">
          ...
          <span class="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium">{{ pendingOrders.length }}</span>
        </div>
        <div class="space-y-2">
      ...
      <div class="bg-white rounded-xl border border-gray-100 p-5">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-base font-semibold text-gray-900">商品明细</h2>
        ...
        <div v-else class="space-y-3">
          <div v-for="(row, i) in rows" :key="i"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <div class="relative">
                  ...
                  <input v-model.number="row.selling_price" type="number" step="0.01" min="0"
                    class="input text-sm pl-7" placeholder="0.00" />
                </div>
              </div>
              <div>
                ...
                <div class="relative">
                  ...
                  <input v-model.number="row.unit_cost" type="number" step="0.01" min="0"
                    class="input text-sm pl-7" placeholder="不确定可留空"
                    @input="row.is_estimated = row.unit_cost <= 0" />
                </div>
              </div>
            ...
            <div>
              ...
              <input v-model="row.remark" type="text" class="input text-sm" placeholder="可选" />
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 结果 -->
      <div v-if="result" class="rounded-lg p-4" :class="result.error ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'">
        <p class="font-medium">{{ result.error ? '操作失败' : '操作成功' }}</p>
        <p v-if="result.error" class="text-sm mt-1">{{ result.error }}</p>
        <p v-else class="text-sm mt-1">{{ isOrderedOnly ? '已下单，等待到货' : '已成功入库' }}</p>
      </div>
    </div>
...
function hideSuggestions(i: number) {
  ...
  try {
    ...
    for (const row of rows.value) {
      if (row.is_new && !row.product_id) {
        ...
        if (error) {
          result.value = { error: `创建商品「${row.product_name}」失败: ${error}` }
          saving.value = false
          return
        }
    ...
    result.value = { error: e?.message ?? '操作失败' }
  } finally {
    saving.value = false
  }
}
...
async function handleReceive(orderId: number) {
  // Load order items for price adjustment
...
async function confirmReceive() {
  ...
  for (const item of receiveDialog.items) {
    priceOverrides[item.product_id] = {
...
async function handleAddSupplier() {
  ...
  addSupplierSuccess.value = false
  ...
  if (!error && data) {
    ...
    // Clear form for next entry, but keep dialog open briefly to show success
