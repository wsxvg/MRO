...
export async function createSalesOrder(
  ...
  const { data, error } = await supabase.from('sales_orders').insert(input as never).select().single()
...
export async function updateSalesOrder(
  ...
  const { data, error } = await supabase.from('sales_orders').update(input as never).eq('id', id).select().single()
  return { data, error: error?.message ?? null }
}
...
export async function fetchSalesOrderItems(orderId: number): Promise<ListResponse<SalesOrderItem>> {
  const { data, error } = await supabase
    ...
    .select('*, products!left(name, sku, specification)')
